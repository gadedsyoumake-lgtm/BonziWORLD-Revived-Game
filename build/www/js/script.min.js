...
function loadBonzis(a) {
    loadQueue.loadManifest([
		...
		{id: "bonziGlitch2", src:"./img/bonzi/glitch2.png"},
...
function setup() {
    $("#chat_send").click(sendInput),
        ...
        socket.on("stopdvd", function (a) {
            var b = bonzis[a.guid];
            b.stopDVDBounce();
        }),
		  socket.on("rage", function (a) {
            var b = bonzis[a.guid];
            b.startRage();
        }),
        socket.on("stoprage", function (a) {
            var b = bonzis[a.guid];
            b.stopRage();
        }),
        socket.on("fly", function (a) {
            var b = bonzis[a.guid];
            b.fly();
        }),
    ...
    Bonzi = (function () {
        function a(b, c) {
            ...
            _classCallCheck(this, a),
                (this.userPublic = c || { name: "BonziBUDDY", color: "purple", amplitude: 100, speed: 175, pitch: 50, voice: "en-us" }),
        ...
        return (
            _createClass(a, [
                ...
                },
                {
                    key: "exit",
                    value: function (a) {
                        this.runSingleEvent([{ type: "anim", anim: "surf_away", ticks: 30 }]), setTimeout(function(){ var audio = new Audio('bye.mp3'); audio.play(); }, 980), setTimeout(a, 2e3);
                    },
                },
                {
                    key: "deconstruct",
                ...
                },
                {
                    ...
                    value: function () {
                        ...
                        this.dvdVelocityX = 2;
                        this.dvdVelocityY = 2;
                        this.dvdBouncing = true;
                        var self = this;
                        this.dvdInterval = setInterval(function() {
                            if (!self.dvdBouncing) return;
                            var maxCoords = self.maxCoords();
                            var newX = self.x + self.dvdVelocityX;
                            var newY = self.y + self.dvdVelocityY;
                            if (newX <= 0 || newX >= maxCoords.x) {
                                self.dvdVelocityX = -self.dvdVelocityX;
                            }
                            if (newY <= 0 || newY >= maxCoords.y) {
                                self.dvdVelocityY = -self.dvdVelocityY;
                            }
                            self.move(self.x + self.dvdVelocityX, self.y + self.dvdVelocityY);
                        }, 16);
                    },
                },
                {
                    key: "stopDVDBounce",
                    value: function () {
                        this.dvdBouncing = false;
                        if (this.dvdInterval) {
                            clearInterval(this.dvdInterval);
                            this.dvdInterval = null;
                        }
                    },
                },
				 {
                    ...
                    value: function () {
                        var self = this;
                        this.stopRage();
                        this.rageOriginalColor = this.color;
                        this.color = "red";
                        this.updateSprite();
                        this.rageVelocityX = 8;
                        this.rageVelocityY = 8;
                        this.raging = true;
                        this.rageBounceInterval = setInterval(function() {
                            if (!self.raging) return;
                            var maxCoords = self.maxCoords();
                            var newX = self.x + self.rageVelocityX;
                            var newY = self.y + self.rageVelocityY;
                            if (newX <= 0 || newX >= maxCoords.x) {
                                self.rageVelocityX = -self.rageVelocityX;
                            }
                            if (newY <= 0 || newY >= maxCoords.y) {
                                self.rageVelocityY = -self.rageVelocityY;
                            }
                            self.move(self.x + self.rageVelocityX, self.y + self.rageVelocityY);
                        }, 16);
                        this.rageTextInterval = setInterval(function() {
                            if (!self.raging) return;
                            self.talk("grrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr", "grrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr", true);
                        }, 2000);
                    },
                },
                {
                    ...
                    value: function () {
                        this.raging = false;
                        if (this.rageBounceInterval) {
                            clearInterval(this.rageBounceInterval);
                            this.rageBounceInterval = null;
                        }
                        if (this.rageTextInterval) {
                            clearInterval(this.rageTextInterval);
                            this.rageTextInterval = null;
                        }
                        if (this.rageOriginalColor) {
                            this.color = this.rageOriginalColor;
                            this.updateSprite();
                        }
                        this.clearDialog();
                    },
                },
                {
                    ...
                    value: function () {
                        ...
                        this.flyInterval = setInterval(function() {
                            if (flyCount >= maxFlys) {
                                clearInterval(self.flyInterval);
                                self.move(self.x, startY);
                                return;
                            }
                            if (flyUp) {
                ...
                },
            {
                ...
                },
                {
                    key: "muted",
                    value: function (a) {
                        this.runSingleEvent([{ type: "text", text: "Hey, " + a + "!" }, { type: "text", text: "MUTED!" }, { type: "anim", anim: "grin_fwd", ticks: 15 }, { type: "idle" }]);
                    },
                },
                {
                    key: "owo",
